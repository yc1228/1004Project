...
unique_value=lines.map(lambda x: (x[8],1)).reduceByKey(add).collect()
#print(unique_value)

def dictionary(x):
    if x[8]== "":
        return (x[8],'Int','KY_CD','Null')
    else:
        if len(x[8])==3:
            return (x[8],'Int','KY_CD','Valid')
        else:
            return (x[8],'Int','KY_CD','Invalid')

dic_value= lines.map(lambda x: dictionary(x)).map(lambda x: x[0]+"\t"+x[1]+"\t"+x[2]+"\t"+x[3]).collect()
dic_value=sc.parallelize(dic_value)
dic_value.saveAsTextFile("Col_PD_CD.out")
