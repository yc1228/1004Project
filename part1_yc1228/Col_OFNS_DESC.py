...
unique_value=lines.map(lambda x: (x[7],1)).reduceByKey(add).collect()
print(unique_value)

def dictionary(x):
    if x[7]== "":
        return (x[7],'TEXT','Description_of_Key_Code','Null')
    else:
        return (x[7],'TEXT','Description_of_Key_Code','Valid')

dic_value= lines.map(lambda x: dictionary(x)).map(lambda x: "{0}\t{1}\t{2}\t{3}".format(x[0], x[1], x[2], x[3]))
#dic_value=sc.parallelize(dic_value)
dic_value.saveAsTextFile("Col_OFNS_DESC.out")
