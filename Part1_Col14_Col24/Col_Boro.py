...
#unique_value=lines.map(lambda x: (x[13],1)).reduceByKey(add).collect()
#print(unique_value)

def dictionary(x):
	if x[13]== "":
		return (x[13],'String','Location','Null')
	else:
		return (x[13],'String','Location','Valid')

dic_value= lines.map(lambda x: dictionary(x)).map(lambda x: x[0]+"\t"+x[1]+"\t"+x[2]+"\t"+x[3]).collect()
dic_value=sc.parallelize(dic_value)
dic_value.saveAsTextFile("Col_Boro.out")